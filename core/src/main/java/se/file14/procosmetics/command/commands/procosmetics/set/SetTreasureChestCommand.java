...
public class SetTreasureChestCommand extends SubCommand<CommandSender> {

    public SetTreasureChestCommand(ProCosmeticsPlugin plugin) {
        super(plugin, "procosmetics.command.set.treasurechest", true);
        addFlats("set", "treasurechest");
    ...
    public void onExecute(CommandSender sender, String[] args) {
        ...
        plugin.getUserManager().getAsync(name).thenAccept(user -> {
            ...
            plugin.getDatabase().setTreasureChestsAsync(user, treasureChest, amount).thenAccept(result -> {
                if (result.firstBoolean()) {
                    audience(sender).sendMessage(translator.translate(
                            "command.set.treasure_chest",
