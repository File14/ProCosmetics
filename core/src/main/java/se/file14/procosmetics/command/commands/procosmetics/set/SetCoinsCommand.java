...
public class SetCoinsCommand extends SubCommand<CommandSender> {

    public SetCoinsCommand(ProCosmeticsPlugin plugin) {
        super(plugin, "procosmetics.command.set.coins", true);
        addFlats("set", "coins");
    ...
    public void onExecute(CommandSender sender, String[] args) {
        ...
        plugin.getUserManager().getAsync(name).thenAccept(user -> {
            ...
            plugin.getEconomyManager().getEconomyProvider().setCoinsAsync(user, amount).thenAccept(result -> {
