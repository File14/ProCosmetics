...
public class DeleteCommand extends SubCommand<CommandSender> {
    ...
    public void onExecute(CommandSender sender, String[] args) {
        ...
        if (platform == null) {
            user.sendMessage(user.translate("command.treasurechest.delete.not_found"));
            return;
        }

        if (platform.isInUse()) {
            user.sendMessage(user.translate("command.treasurechest.delete.in_use"));
            return;
        }
        plugin.getTreasureChestManager().deletePlatform(platform);
        user.sendMessage(user.translate("command.treasurechest.delete.success"));
