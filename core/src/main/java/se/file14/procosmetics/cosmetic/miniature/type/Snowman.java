...
public class Snowman implements MiniatureBehavior {

    private static final ItemStack HAND_ITEM = new ItemStack(Material.SNOWBALL);
    ...
    public void onUpdate(CosmeticContext<MiniatureType> context, NMSEntity nmsEntity, int tick) {
        if (tick % 15 == 0) {
            MathUtil.findLocationsInCircle(nmsEntity.getPreviousLocation(), 8, 0.3d, 1.5d,
                    location -> location.getWorld().spawnParticle(Particle.SNOWFLAKE, location, 0)
            );
        }
    }
