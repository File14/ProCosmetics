...
public class IronGolem implements MiniatureBehavior {

    private static final ItemStack HAND_ITEM = new ItemStack(Material.POPPY);
    ...
    public void onUpdate(CosmeticContext<MiniatureType> context, NMSEntity nmsEntity, int tick) {
        if (tick % 15 == 0) {
            MathUtil.findLocationsInCircle(nmsEntity.getPreviousLocation(), 8, 0.3d, 1.5d,
                    location -> location.getWorld().spawnParticle(Particle.FIREWORK, location, 0)
            );
        }
    }
